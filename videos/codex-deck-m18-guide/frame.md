...
description: >
  ...
  Noto Sans JP in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  ...
  # — display / hero ramp (Noto Sans JP, lowercase, negative tracking) —
...
text and becomes graphic primitive.** Noto Sans JP `display` at 13cqw puts a single lowercase word
...
**Noto Sans JP** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Noto Sans JP 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Noto Sans JP body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Noto Sans JP `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Noto Sans JP display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Noto Sans JP `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Noto Sans JP lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Noto Sans JP `display`/`h1`
...
row of three top-border-only stat-cards — big Noto Sans JP-900 numeral in `{colors.fire-orange}`, Noto Sans JP
...
**Ground** ink-black. **Composes** fadelist (3 stacked Noto Sans JP-900 words at 1.0/0.5/0.22), fadelist-title.
...
Noto Sans JP `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Noto Sans JP display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Noto Sans JP display; never add a second accent color.
...
- No serif companion; chrome is never Noto Sans JP.
...
- **Type** — Noto Sans JP lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Noto Sans JP + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
...
## 日本語版・DESIGN.md 由来のレイアウト規範（この動画専用の追補）

ユーザー指定の `DESIGN.md`（Mizuno デザイン仕様）は**レイアウト規範としてのみ**採用する。配色は上記フロントマターの M18／Codex 系（ダーク基調 + ティール `#2FD8C6`）が常に優先で、`#001489` などミズノのブランド色は使用しない。

採用する規範:

- **フラットな面** — カード・パネルに影を落とさない（elevation 0）。面の階層は背景色の差と 1px ハイrailine で表す。
- **角丸は小さく** — パネル・カード・ボタンの radius は 3px。例外は tag pill のみで 30px（フル pill）。
- **本文の行間 1.67** — 日本語本文（`body` / `lead`）は line-height 1.67 を下回らない。見出しは 1.1〜1.2 のまま。
- **字間** — `letter-spacing: normal`、`font-feature-settings: "palt"` は使用しない（和文フォント + 大サイズでは字面が崩れるため）。
- **余白スケール** — 3 / 5 / 7 / 18 / 24 / 48px を基準単位とし、cqw 指定に落とすときもこの比率を保つ。
- **CTA / ラベルの二値構成** — 「濃地に明字」か「明地に濃字」のどちらかに寄せ、中間トーンのボタンを作らない。
- **改行** — `word-break: break-word` / `overflow-wrap: break-word` を前提に、和文は文節で折る。

日本語タイポの注意:

- display / h1 の `lower: true`（小文字化）は**欧文にのみ**適用する。和文には効かないため、和文見出しは weight と大きさで押す。
- 和文本文は 900 ウェイトを使わない（可読性が落ちる）。400〜700 に収める。

## この動画で使う実素材

- `public/keys/*.png` — 実配布のキー画像（ピクセルアート）。144×144 相当、ダークストーン + 金 + ティール発光。ティール `#2FD8C6` と自然に馴染む。
- `public/sheets/*.png` — Codex Micro / Codex Tools 各15面の一覧シート。
- Agent ステータス色（`src/render.ts` の `SIGNAL_COLORS` 実値、改変禁止）:
  未割当 `#000000` / 待機 `#FFFFFF` / 作業中 `#304FFE` / 完了未読 `#00FF4C` / 入力待ち `#FF6D00` / エラー `#FF0033`。
  これらはステータス表示にのみ使用し、地の装飾には使わない。
